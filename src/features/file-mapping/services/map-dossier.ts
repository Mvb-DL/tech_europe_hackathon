...
import type { FileClassificationAgent, FileFingerprint, FileMap, FileMapGroup, FileMappingEvent } from "../domain/contracts";
import { classifyWithRules, createDynamicGroupResult } from "../classification/rule-classifier";
...
export async function mapDossier(dossierId: string, agent: FileClassificationAgent | null = createOpenAiClassificationAgent()) {
  ...
  const results = fingerprints.map(classifyWithRules);
  for (const result of results) {
    if (result.primaryDomain !== "unknown" || result.groupId) continue;
    const fingerprint = fingerprints.find((current) => current.fileId === result.fileId);
    if (!fingerprint) continue;
    const dynamicResult = createDynamicGroupResult(fingerprint);
    Object.assign(result, dynamicResult);
    event("file.dynamic_group.created", `Created dynamic group ${dynamicResult.groupLabel}.`, result.fileId, { groupId: dynamicResult.groupId, groupLabel: dynamicResult.groupLabel, groupReason: dynamicResult.groupReason });
  }
  const ambiguous = results.filter((result) => result.confidence < 0.9 && !result.groupId);
  ...
  const groupKeys = [...new Set(results.map((result) => result.groupId ?? `group:${result.primaryDomain}`))];
  const groups: FileMapGroup[] = groupKeys.map((groupId) => {
    const groupResults = results.filter((result) => (result.groupId ?? `group:${result.primaryDomain}`) === groupId);
    const first = groupResults[0];
    const domain: FileMapGroup["domain"] = first.groupId ? "custom" : first.primaryDomain;
    return { id: groupId, domain, label: first.groupLabel ?? domainLabels[first.primaryDomain], reason: first.groupReason, fileIds: groupResults.map((result) => result.fileId) };
  });
  ...
  results.forEach((result) => { event("file.classified", "Classified file.", result.fileId, { result }); if ((result.primaryDomain === "unknown" && !result.groupId) || result.confidence < 0.65) event("file.review.required", "File requires review.", result.fileId, { warnings: result.warnings }); });
  ...
  const now = stamp(); const fileMap: FileMap = { dossierId, createdAt: now, updatedAt: now, files: results, groups, connections, summary: { totalFiles: results.length, mappedFiles: results.filter((result) => result.primaryDomain !== "unknown" || Boolean(result.groupId)).length, unknownFiles: results.filter((result) => result.primaryDomain === "unknown" && !result.groupId).length, agentClassifiedFiles: results.filter((result) => result.method === "agent").length, reviewRequiredFiles: results.filter((result) => (result.primaryDomain === "unknown" && !result.groupId) || result.confidence < 0.65).length } };
